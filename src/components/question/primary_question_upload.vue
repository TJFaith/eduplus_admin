                ...
                <div class="modal-content">
                    ...
                    <div class="from-group row">
                      ...
                      <quill-editor ref="option"
                       ...
                       <button type="button" @click="currentOption = '', saveQuestion.answers_id='', optionStatusText='ADD OPTION', optionBtnDisable = true " :disabled="optionBtnDisable" class="btn btn-light  mb-2">NEW OPTION</button>
                        </div>
...
export default {
    ...
    methods:{
        ...
        getAnswer(q_id, answer_id, count){
            ...
            this.axios.post(this.$hostname+"primary_api.php?action=getAnswer",question_id).then((response)=>{
       ...
       getAllQuestions(){
            let subject_id = {'subject_id':this.$session.get('primary_subject')};
             this.axios.post(this.$hostname+"primary_api.php?action=getAllQuestions",subject_id).then((response)=>{
        ...
        addQuestion(){
            if(this.saveQuestion.questions == undefined){
                alert('Type in your question');
               
            }else if(this.saveQuestion.answer==undefined || this.saveQuestion.answer ==''){
                alert("You haven't selected any answer yet, clikc on 'ADD OPTIONS' to do that")
            }else{
            this.feedBack='Uploading....'
            this.showSpinner= false;
            this.saveQuestion.subject_id = this.$session.get('primary_subject')
            ...
            if(!Object.prototype.hasOwnProperty.call(this.saveQuestion,'answer')){
             ...
             this.axios.post(this.$hostname+"primary_api.php?action=save_question",this.saveQuestion).then((response)=>{
        ...
        newQuestion(){
          ...
          this.saveQuestion={};
          this.saveQuestion.question_id= this.generateID('p_qst');
          this.currentOption = '';
        ...
        deleteFunc(question_id){
             ...
             this.axios.post(this.$hostname+"primary_api.php?action=deleteQuestion",qustion_id).then((response)=>{
      ...
      async editFunc(question_id){
           
             let question_id_obj = {'question_id':question_id}
            this.axios.post(this.$hostname+"primary_api.php?action=getOneQuestion",question_id_obj).then((response)=>{
           
                    let questionData = response.data;
                  let answer_id_arr = {'question_id':question_id, 'answer_id':response.data[0].answers_id}
                  this.axios.post(this.$hostname+"primary_api.php?action=getAnswer",answer_id_arr).then((response2)=>{
                    ...
                    // this.saveQuestion.answer = this.answer;
        ...
        getInstructions(){
                    let subject_id = {'subject_id':this.$session.get('primary_subject')};
                    this.axios.post(this.$hostname+"primary_api.php?action=getInstructions",subject_id).then((response)=>{
        ...
        saveInstruction(){
            let instruction_data = {'subject_id':this.$session.get('primary_subject'),'instruction_id':this.selected_ins.id,'instruction':this.selected_ins.text};
            this.axios.post(this.$hostname+"primary_api.php?action=saveInstruction", instruction_data).then((response)=>{
        ...
        deleteInstructon(instruction_id){
           let instruction_id_obj = {'instruction_id':instruction_id}
                     this.axios.post(this.$hostname+"primary_api.php?action=deleteInstruction", instruction_id_obj).then((response)=>{
        ...
        newInstruction(){
           ...
           this.showEditor = true;
            this.selected_ins.id=this.generateID("p_ins");
        //    
        },
        ...
        // START ======================= METHODS FOR OPTIONS
            getOptionsFromDB(){
                    ...
                    this.axios.post(this.$hostname+"primary_api.php?action=getQuestionOptions",option_data).then((response)=>{
        ...
        addOption(){
            ...
            if(this.currentOption != '' & this.optionStatusText != 'UPDATE'){
                let option_data = {'question_id':this.saveQuestion.question_id, 'options':this.currentOption};  
                 this.axios.post(this.$hostname+"primary_api.php?action=saveOptions",option_data).then((response)=>{
                     response;
                 ...
                 this.axios.post(this.$hostname+"primary_api.php?action=updateOption",option_data).then((response)=>{
                     response
                    this.getOptionsFromDB();
                 })
                this.optionStatusText='ADD OPTION';
        ...
        deleteOpton(){
            let option_data = {'option_id':this.saveQuestion.answers_id};  
                 this.axios.post(this.$hostname+"primary_api.php?action=deleteOption",option_data).then((response)=>{
                     response
                    this.getOptionsFromDB();
            })
